...
create or replace table managed_table_with_default_location(width int, length int, height int);
INSERT into managed_table_with_default_location values (3, 2, 1);
select * from managed_table_with_default_location;
...
DESCRIBE EXTENDED managed_table_with_default_location;
...
-- MAGIC
-- MAGIC
-- MAGIC table_loc = spark.sql(f"Describe detail managed_table_with_default_location").first().location
-- MAGIC print(table_loc)
-- MAGIC
-- MAGIC files = dbutils.fs.ls(table_loc)
-- MAGIC display(files)
...
DROP TABLE managed_table_with_default_location;
...
-- MAGIC
-- MAGIC
-- MAGIC schema_def_loc = spark.sql(f"DESCRIBE schema {DA.schema_name}_default_location").collect()[3].database_description_value
-- MAGIC print(schema_def_loc)
-- MAGIC dbutils.fs.ls(schema_def_loc)
-- MAGIC # display(spark.sql(f"DESCRIBE schema {DA.schema_name}_default_location").collect()[3])
