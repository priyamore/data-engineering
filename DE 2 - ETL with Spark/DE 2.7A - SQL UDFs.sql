...
create or replace function custom_udf_sales_ad(item_name string, item_price int)
returns string
return concat("The ", item_name, "is on sale for $", round(item_price * 0.8, 0));

select *, custom_udf_sales_ad(name, price) as message from item_lookup;

-- COMMAND ----------

select distinct name from item_lookup
...
DESCRIBE FUNCTION EXTENDED custom_udf_sales_ad
...
create or replace FUNCTION item_pref(name STRING, price int)
returns STRING
RETURN case
  WHEN name = "Standard Queen Mattress" then "This is my default mattress"
  when name = "Premium Queen Mattress" THEN "This is my favorite mattress"
  when name = "Standard Down Pillow" then "This is my fav pillow"
  when price > 100 then concat("I am going to wait until the ", name, " is on sale for $", round(0.8*price,0))
  else concat("I dont need this ",  name)
end;

select *, item_pref(name, price) FROM item_lookup;
