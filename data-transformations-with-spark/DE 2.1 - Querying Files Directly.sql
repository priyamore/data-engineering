...
select * from json.`${DA.paths.kafka_events}/002.json`;
...
create or replace view event_view_test as select * from json.`${DA.paths.kafka_events}`
...
create or replace temp view temporary_view_for_events as select * from json.`${DA.paths.kafka_events}`
...
with cte_random as (select * from json.`${DA.paths.kafka_events}`) select * from cte_random
