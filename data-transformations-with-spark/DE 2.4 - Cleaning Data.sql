...
create or replace temp view deduped_records as select user_id, user_first_touch_timestamp, max(email) as one_email, max(updated) as one_updated from users_dirty where user_id is not null group by user_id, user_first_touch_timestamp;

-- COMMAND ----------

select count(*) from deduped_records;
...
-- MAGIC
-- MAGIC user_records_df = spark.read.table("users_dirty")
-- MAGIC
-- MAGIC deduped_records_df = (user_records_df
-- MAGIC                       .where(col("user_id").isNotNull())
-- MAGIC                       .groupBy("user_id", "user_first_touch_timestamp")
-- MAGIC                       .agg(max('email').alias('one_email'), 
-- MAGIC                            max('updated').alias('one_updated'))
-- MAGIC                       )
-- MAGIC
-- MAGIC # display(deduped_records_df)
-- MAGIC deduped_records_df.count()
-- MAGIC
...
-- MAGIC %python
-- MAGIC
-- MAGIC # user_records_df.dropDuplicates(["user_id", "user_first_touch_timestamp"]).filter(col("user_id").isNotNull()).count()
-- MAGIC
-- MAGIC user_records_df.dropDuplicates(["user_id", "user_first_touch_timestamp"]).where(col("user_id").isNotNull()).count()
...
select max(row_count) <= 1 no_duplicate_ids_found from (
...
-- MAGIC         .select(max("row_count") <= 1).alias("no_duplicate_ids_found")
-- MAGIC         )
-- MAGIC
...
select max(user_id_count) <= 1 at_most_id_per_user from (
  select one_email, count(user_id) as user_id_count
  from deduped_records
  where one_email is not null
  group by one_email
 )
...
-- MAGIC
-- MAGIC display(
-- MAGIC     deduped_records_df
-- MAGIC     .where(col("one_email").isNotNull())
-- MAGIC     .groupBy("one_email")
-- MAGIC     .agg(count("user_id").alias("user_id_count"))
-- MAGIC     .select((max("user_id_count") <= 1).alias("at_most_one_id_per_user"))
-- MAGIC )
...
select *, date_format(first_created, "MMM d, yyyy") as first_created_date,  date_format(first_created, "HH:mm:ss") as first_created_time, regexp_extract(one_email, "(?<=@).+", 0) as email_domain
from (
  select *, cast(user_first_touch_timestamp/1e6 as timestamp) as first_created
  from deduped_records
)
...
-- MAGIC display(
-- MAGIC     deduped_records_df
-- MAGIC     .withColumn("first_created", (col("user_first_touch_timestamp") / 1e6).cast("timestamp"))
-- MAGIC     .withColumn("first_create_date", date_format("first_created", "MMM d, yyyy"))
-- MAGIC     .withColumn("first_create_time", date_format("first_created", "HH:mm:ss"))
-- MAGIC     .withColumn("email_domain", regexp_extract("one_email", "(?<=@).+", 0))
-- MAGIC )
-- MAGIC
