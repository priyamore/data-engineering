...
-- MAGIC import pyspark.sql.functions as F
-- MAGIC
-- MAGIC loc_data_type = spark.sql("describe extended users_jdbc").filter(F.col("col_name") == "Location").first()["data_type"]
-- MAGIC # print("loc_data_type:", loc_data_type)
-- MAGIC
-- MAGIC files_at_loc = dbutils.fs.ls(loc_data_type)
-- MAGIC print("number of files at loc:", len(files_at_loc), "files: ", files_at_loc)
-- MAGIC # location
